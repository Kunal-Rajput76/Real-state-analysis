...
most_expensive_bhk=df.groupby('flat_type')['rate_per_sqft'].mean().idxmax()
# print(f"The most expensive property type is {most_expensive_bhk}.")



# question 8:Which property type (Apartment, Floor, Plot) is the costliest on per sqft basis?
...
top_5_builders=df.groupby('company_name')['price'].mean().sort_values(ascending=False).head(5)
for builder in top_5_builders.index:
    avg_price = top_5_builders[builder]
    print(f"{builder} has an average property price of {avg_price/10000000:.2f} crores.")
